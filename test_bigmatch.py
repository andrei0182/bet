...
try:
    ...
    opened = open_over_under_tab(driver, wait_seconds=30.0)
    ...
    if opened:
        ...
        if table and away_id:
            row_data = parse_team_row_from_table(table, away_id)
            print(f"[{time.time()-t0:.1f}s] away row for 2.5: {row_data}")
finally:
    driver.quit()
